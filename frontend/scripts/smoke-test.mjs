const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4610';
...
const adminCookie = await login('admin@quality-safety.local', 'admin123');
const managerCookie = await login('manager@quality-safety.local', 'manager123');
const analystCookie = await login('analyst@quality-safety.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/incidents', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/incidents', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/incidents/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/incidents/approve`, {
  method: 'POST',
  headers: {
